...
public class Phase8Setup
{
    ...
    private static void SetupWaypointsAndOutput()
    {
        ...
        // Add Stopper Sensor Visuals
        Transform stopperSensor = inspectionApproach.Find("EntryStopperSensor");
        if (stopperSensor == null)
        {
            stopperSensor = GameObject.CreatePrimitive(PrimitiveType.Cube).transform;
            stopperSensor.name = "EntryStopperSensor";
            stopperSensor.SetParent(inspectionApproach, false);
            stopperSensor.localPosition = new Vector3(0.5f, 0.1f, 0);
            stopperSensor.localScale = new Vector3(0.1f, 0.2f, 0.1f);
            UnityEngine.Object.DestroyImmediate(stopperSensor.GetComponent<Collider>());
            stopperSensor.gameObject.AddComponent<StopperSensorController>();
        }
