...
public class PCBSpawner : MonoBehaviour
{
    ...
    [Tooltip("Minimum distance required between PCBs.")]
    public float minimumPCBSpacing = 2.5f;
    ...
    private GameObject lastSpawnedPCB = null;
    ...
    private void Update()
    {
        ...
        if (timer >= spawnInterval)
        {
            ...
            {
                bool canSpawn = true;
                if (lastSpawnedPCB != null)
                {
                    float distance = Vector3.Distance(transform.position, lastSpawnedPCB.transform.position);
                    if (distance < minimumPCBSpacing)
                    {
                        canSpawn = false;
                    }
                }
